...
@php
    ...
    $authQuery = $redirect ? '?redirect='.urlencode($redirect) : '';
@endphp
<div class="mx-auto flex min-h-[60vh] max-w-md items-center px-4 py-12">
    <div class="card w-full p-8">
        <h1 class="text-2xl font-bold text-center">{{ __('messages.login') }}</h1>
        <p class="mt-2 text-center text-sm text-[var(--color-text-muted)]">{{ __('messages.tagline') }}</p>

        <div class="mt-8 space-y-3">
            <a href="{{ route('auth.redirect', 'google') }}{{ $authQuery }}" class="btn-secondary w-full">
            ...
            <a href="{{ route('auth.redirect', 'facebook') }}{{ $authQuery }}" class="btn-secondary w-full">
                <svg class="h-5 w-5" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                Facebook
            </a>
            <a href="{{ route('auth.redirect', 'apple') }}{{ $authQuery }}" class="btn-secondary w-full">
        ...
        <p class="mt-6 text-center text-xs text-[var(--color-text-muted)]">
            Само вход чрез социални мрежи · Social login only
        </p>
    </div>
</div>
@endsection