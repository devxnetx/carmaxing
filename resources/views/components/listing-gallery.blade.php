...
@php
    $imageUrls = $listing->images->map(fn ($img) => [
        'large' => $img->url('large'),
...
@if($listing->images->isNotEmpty())
    <div class="listing-print-gallery mb-6 hidden gap-3 print:grid">
        @foreach($listing->images->take(8) as $image)
            <img
                src="{{ $image->url('large') }}"
                alt="{{ $listing->composeDisplayTitle() }}"
                class="h-auto w-full rounded border border-gray-200 object-cover"
                style="max-height: 14rem; break-inside: avoid;"
            >
        @endforeach
    </div>
@endif
...
<div
    class="card no-print overflow-hidden"
...
>
    ...
    >
        ...
        <x-new-ad-badge :listing="$listing" class="pointer-events-none absolute left-3 top-3 z-10" />
